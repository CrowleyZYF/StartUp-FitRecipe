...
public class PluginSimplePlayer extends PluginOverlay implements DetailMessage {
	...
	private Button quality_Button;// 全屏按钮
	...
	private void initPlayLayout() {
		...
		quality_Button = (Button) containerView
				.findViewById(R.id.ib_quality);

		quality_Button.setOnClickListener(new OnClickListener() {

			@Override
			public void onClick(View v) {
				if(quality_Button.getText().equals("高清")) {
					int result = ApiManager.getInstance().changeVideoQuality(VideoQuality.SUPER, mBasePlayerManager);
					if(result == 1)
						quality_Button.setText("标清");
				}else {
					int result = ApiManager.getInstance().changeVideoQuality(VideoQuality.STANDARD, mBasePlayerManager);
					if(result == 1)
						quality_Button.setText("高清");
				}
			}
		});
...
//			}
	...
	private void disableController() {
		...
		setClickable(false, quality_Button);
		setClickable(false, videoBar);
		hideTitle();
	}
	...
	private void enableController() {
		...
		setClickable(true, quality_Button);
		setClickable(true, videoBar);
	}
